...
class SatSolver
{
   public : 
      ...
      void addNandCNF(Var vo, vector<Var> vi)
      {  
         assert(!vi.empty());
      ...
      void addNorCNF(Var vo, vector<Var> vi )
      {
         assert(!vi.empty());
      ...
      void addXnorCNF(Var vf, Var va, Var vb)
      {
         vec<Lit> lits;
         Lit lf = Lit(vf);
         Lit la = Lit(va); Lit lb = Lit(vb);
      ...
      void addNotCNF(Var vo, Var vi)
      {
         vec<Lit> lits;
         Lit lo = Lit(vo);
         Lit li = Lit(vi);

         lits.push(li); lits.push(lo);
         _solver->addClause(lits); lits.clear();
